...
def preprocess_data(data, input_template=None, input_key=None, output_key=None):
    ...
    if input_key and output_key:
        ...
        # jondurbin/airoboros-3.2
            ...
            input_template = None  # do not modified with input template again
        # chargoddard/commitpack-ft-instruct; alpaca; open-platypus #bak
        elif exist_and_not_none(data, "instruction") and exist_and_not_none(data, "output") \
            and exist_and_not_none(data, "input"):
            if len(data["input"])>=1:
...
class SFTDatasetMixture(Dataset):
    """
    ...
    def __init__(
        ...
        for data in tqdm(dataset, disable=not self.strategy.is_rank_0()):
            ...
            self.weights.append(dataset_weight)
            
        for data in tqdm(dataset2, disable=not self.strategy.is_rank_0()):
            prompt, response = preprocess_data(data, None if pretrain_mode else input_template, input_key, output_key)
            ...
            self.prompts.append(prompt)
            self.responses.append(response)
            self.weights.append(1-dataset_weight)
    ...
    def __getitem__(self, idx):
        ...
        return prompt_ids_len, input_token["input_ids"], input_token["attention_mask"], w, info
    ...
    def collate_fn(self, item_list):
        ...
        for prompt_ids_len, input_id, attention_mask, w, info in item_list:
        ...
        return prompt_ids_lens, input_ids, attention_masks, weights, infos
